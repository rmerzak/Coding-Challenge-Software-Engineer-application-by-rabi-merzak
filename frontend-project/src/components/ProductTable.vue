    ...
    <div>
        ...
        <thead>
          <tr>
            <th v-for="(col, index) in head" :key="index" scope="col">
                {{ col }}
                <button @click="updateSortOrder('asc')" v-if="col === 'Price'">&#9650;</button>
                <button @click="updateSortOrder('desc')" v-if="col === 'Price'">&#9660;</button>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in paginatedData" :key="row.id && index">
            ...
            <td>{{ row.price }}</td>
            <td><img :src="require('/usr/src/app/public/' + row.image)" width="100" height="100" /></td>
            <td>
              <ul>
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          ...
          <li class="page-item" v-for="pageNumber in pageNumbers" :key="pageNumber" :class="{ active: currentPage === pageNumber, disabled: pageNumber === '...' }">
  ...
  export default {
    ...
    computed: {
      ...
      paginatedData() {
        ...
        return this.sortedProduct.slice(startIndex, endIndex);
      },
      sortedProduct() {
